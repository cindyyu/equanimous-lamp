<new>
	<logo />
	<form class='grid' onsubmit={ addSpot }>
		<div class='grid__col col--1-of-2'>
			<label for='location'>Where is this awesome parking spot?</label>
		...
		<div class='grid__col col--1-of-2'>
			<label for='price'>Is it always free?</label>
			<button data-type='free' class={ type == 'free' ? 'selected' : '' } onclick={ setType }>Yes</button>
			<button data-type='paid' class={ type == 'paid' ? 'selected' : '' } onclick={ setType }>No</button>

			<paid if={ type == 'paid' }>
				<label>When do you have to pay?</label>
				<availability>
					<day each={ days } class={ selected ? 'selected' : '' } onclick={ toggleDay }>{ name }</day>
					<time>
						from
						<beg>
							<select onchange={ updateField } name='beg_hour' value={ self.beg_hour }>
								<option each={ hours }>{ value }</option>
							</select>:
							<select onchange={ updateField } name='beg_min' value={ self.beg_min }>
								<option each={ minutes }>{ value }</option>
							</select>
						</beg>
						to
						<end>
							<select onchange={ updateField } name='end_hour' value={ self.end_hour }>
								<option each={ hours }>{ value }</option>
							</select>:
							<select onchange={ updateField } name='end_min' value={ self.end_min }>
								<option each={ minutes }>{ value }</option>
							</select>
						</end>
					</time>
				...
				<label for='price'>How much does it cost per hour?</label>
				<i class='material-icons'>attach_money</i><input type='text' name='price' />

			</paid>

			<label for='max_stay'>How many hours can you park here?</label>
			<i class='material-icons'>av_timer</i><input type='text' name='max_stay' />

			<button disabled={ added }>{ added ? 'Got it!' : 'Add Parking Spot' }</button>
			<button if={ added } onclick={ clearForm }>Add Another</button>
		</div>
	</form>
